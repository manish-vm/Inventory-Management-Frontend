...
const ReviewRoutingSection = ({
  config,
  setConfig,
  currentStage,
  stages = [],
  configurationMode = 'stages',
  onCreateQuestionnaire,
  onCreateReworkQuestionnaire,
  onCreateOkQuestionnaire
}) => {
  ...
  if (configurationMode === 'finalStages') {
    return (
      <div className="grid gap-4 xl:grid-cols-2">
        <section className="rounded-lg border border-emerald-200 bg-white p-5 shadow-sm dark:border-emerald-900 dark:bg-slate-800">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">OK Track</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Move product after OK approval.</p>
            </div>
          </div>
          <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">
            On OK, move from {stageLabel(currentStage)} to
            <select
            ...
            >
              <option value="">Select final stage</option>
        ...
        <section className="rounded-lg border border-red-200 bg-white p-5 shadow-sm dark:border-red-900 dark:bg-slate-800">
          <div className="mb-4 flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-slate-900 dark:text-white">Not OK Track</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">Collect the final failure or not-OK response details.</p>
            </div>
          </div>
          <button
            type="button"
          ...
          >
            {config.rejectionQuestionnaireEnabled ? 'Edit Not OK Questionnaire' : '+ Create Not OK Questionnaire'}
          </button>
        </section>
      </div>
    );
